    ...
    <template v-else>
      <div class="grid grid-cols-2 gap-2 mb-3 p-3 bg-gray-900 rounded border border-gray-700">
        <div>
          ...
          <input
            :value="book.name ?? ''"
            @input="book.name = ($event.target as HTMLInputElement).value || undefined"
        ...
        <div>
          ...
          <input
            ...
            @input="book.description = ($event.target as HTMLInputElement).value || undefined"
            class="w-full px-2 py-1 text-xs bg-gray-800 border border-gray-600 rounded text-gray-200"
          />
        </div>
        <div>
          ...
          <input
            ...
            @input="
              book.scan_depth = Number(($event.target as HTMLInputElement).value) || undefined
            "
            type="number"
            class="w-full px-2 py-1 text-xs bg-gray-800 border border-gray-600 rounded text-gray-200"
          />
        ...
        <div>
          ...
          <input
            ...
            @input="
              book.token_budget = Number(($event.target as HTMLInputElement).value) || undefined
            "
            type="number"
            class="w-full px-2 py-1 text-xs bg-gray-800 border border-gray-600 rounded text-gray-200"
          />
        ...
        <div class="flex justify-items-center pb-1">
          <label class="flex items-center gap-1 text-xs text-gray-400">
            <input
              ...
              @change="
                book.recursive_scanning = ($event.target as HTMLInputElement).checked || undefined
              "
            />
            Recursive Scanning
          </label>
      ...
      <div ref="entryListRef" class="relative">
        <TransitionGroup name="list">
          <div v-for="(entry, index) in book.entries" :key="entryKeys[index]">
            <EntryCard
              :entry="entry"
              :index="index"
