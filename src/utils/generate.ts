...
const DEFAULT_PROMPTS: Record<GenerateField, { generate: string; polish: string }> = {
  description: {
    generate:
      'Write a character description for {{char}}. Include their appearance, demeanor, background, and notable traits. Write in third person.',
    polish:
      'Improve this character description to be more vivid and detailed. Keep the established identity and facts intact.',
  },
  personality: {
    generate:
      "Describe {{char}}'s personality — their traits, quirks, mannerisms, values, and behavioral patterns. Be specific and nuanced.",
    polish:
      'Improve this personality description to be more nuanced and specific. Preserve the established traits while adding depth.',
  },
  scenario: {
    generate:
      'Write an opening scenario describing where and how {{char}} meets {{user}}. Set the mood and establish context.',
    polish:
      'Improve this scenario to be more atmospheric and engaging. Keep the core situation intact.',
  },
  mes_example: {
    generate:
      "Write example dialogue showing {{char}}'s voice and personality. Use '{{char}}: dialogue' and '{{user}}: dialogue' format.",
    polish:
      "Improve this example chat to better capture {{char}}'s voice. Keep core interactions, enhance characterization through dialogue.",
  },
  greeting: {
    generate:
      "Write a greeting message for {{char}} — their opening line to {{user}}. Establish the character's voice and set the scene.",
    polish:
      'Improve this greeting to be more in-character and engaging. Keep the core message intact.',
  },
  lore: {
    generate:
      'Write lore content for this entry based on its key triggers: {{keys}}. Provide worldbuilding or character background that enriches the roleplay.',
    polish:
      'Improve this lore entry to be more detailed and useful. Keep the core information while adding depth.',
  },
};
...
export function loadPromptMemory(field: GenerateField, mode: GenerateMode): string | null {
  try {
    return localStorage.getItem(memoryKey(field, mode));
  } catch {
    return null;
  }
}
...
export function savePromptMemory(field: GenerateField, mode: GenerateMode, prompt: string): void {
  try {
    localStorage.setItem(memoryKey(field, mode), prompt);
  } catch {}
}

export function clearPromptMemory(field: GenerateField, mode: GenerateMode): void {
  try {
    localStorage.removeItem(memoryKey(field, mode));
  } catch {}
}
...
export function assembleGeneratePrompt(
  ...
  const defaultMain =
    'You are a writing assistant for authoring character cards for roleplay. ' +
  ...
  if (data.description) blocks.push(`=== Description ===\n${data.description}`);
  if (data.personality) blocks.push(`=== Personality ===\n${data.personality}`);
  if (data.scenario) blocks.push(`=== Scenario ===\n${data.scenario}`);
  if (data.mes_example) blocks.push(`=== Example Chat ===\n${data.mes_example}`);
