...
const showResultReview = computed(
  () => resultText.value.trim() !== '' && !generating.value && done.value,
);
...
async function handleGenerate() {
  ...
  try {
    localStorage.setItem(memoryKey('generateTheme'), theme.value);
  } catch {}
...
<template>
  <Teleport to="body">
    ...
    >
      <div
        class="bg-gray-900 border border-gray-700 rounded-lg w-full max-w-2xl max-h-[85vh] flex flex-col"
      >
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-4">
          <div v-if="hasCharContext" class="space-y-1">
            ...
            <div v-if="store.cardJson?.data.description" class="text-xs text-gray-300 flex gap-2">
              ...
              <span class="truncate text-gray-500"
                >— {{ store.cardJson.data.description.slice(0, 80) }}</span
              >
            </div>
            <div v-if="store.cardJson?.data.personality" class="text-xs text-gray-300 flex gap-2">
              ...
              <span class="truncate text-gray-500"
                >— {{ store.cardJson.data.personality.slice(0, 80) }}</span
              >
            </div>
            <div v-if="store.cardJson?.data.scenario" class="text-xs text-gray-300 flex gap-2">
              ...
              <span class="truncate text-gray-500"
                >— {{ store.cardJson.data.scenario.slice(0, 80) }}</span
              >
            </div>
            <div v-if="store.cardJson?.data.mes_example" class="text-xs text-gray-300 flex gap-2">
              ...
              <span class="truncate text-gray-500"
                >— {{ store.cardJson.data.mes_example.slice(0, 80) }}</span
              >
            </div>
          </div>
            ...
            <div class="space-y-1 overflow-y-auto">
              <template v-for="(e, i) in loreEntries" :key="i">
                ...
                >
                  ...
                  <span v-if="e.content" class="truncate text-gray-500"
                    >— {{ e.content.slice(0, 50) }}</span
                  >
                </label>
              </template>
            </div>
          ...
          <div>
            <div class="flex items-center justify-between mb-2">
              ...
              <button class="text-xs text-gray-500 hover:text-gray-300" @click="handleReset">
                ↺ Reset
              </button>
            </div>
            <MarkdownField v-model="userPrompt" :disabled="generating" />
          </div>

          <div>
            <label class="text-xs text-gray-400 block mb-1">Theme</label>
            <MarkdownField v-model="theme" :disabled="generating" />
          </div>
