...
function onTypeChange(row: ExtensionRow) {
  if (row.type === 'string')
    row.value = typeof row.value === 'string' ? row.value : String(row.value ?? '');
    ...
    <div v-else class="space-y-1">
      <div v-for="row in rows" :key="row.id" class="flex items-center gap-1">
        ...
        <select
          :value="row.type"
          @change="
            row.type = ($event.target as HTMLSelectElement).value as ExtensionRow['type'];
            onTypeChange(row);
            emitIfKeyed(row);
          "
        ...
        <template v-else-if="row.type === 'number'">
          <input
            ...
            @input="
              row.value = Number(($event.target as HTMLInputElement).value) || 0;
              emitIfKeyed(row);
            "
            type="number"
            class="flex-1 px-2 py-1 text-xs bg-gray-800 border border-gray-600 rounded text-gray-200 font-mono"
        ...
        <template v-else-if="row.type === 'boolean'">
          <label class="flex items-center gap-1 text-xs text-gray-400 flex-1">
            <input
              ...
              @change="
                row.value = ($event.target as HTMLInputElement).checked;
                emitIfKeyed(row);
              "
            />
