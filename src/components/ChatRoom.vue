  ...
  (newInfo, oldInfo) => {
    ...
    if (newMsgs.length > oldMsgs.length) {
      ...
      while (diffIdx < newMsgs.length) {
        if (
          newMsgs[diffIdx].role !== oldMsgs[diffIdx].role ||
...
async function mockStreamResponse() {
  ...
  try {
    ...
    for await (const chunk of mockStreamText(
...
<template>
  <div class="flex flex-col h-full">
    <div class="relative">
      ...
      <div
        v-if="showSessions"
        class="absolute left-0 right-0 z-30 mt-1 border border-gray-700 rounded bg-gray-800 max-h-40 overflow-y-auto shadow-lg"
      >
        <div
    ...
    <div ref="chatEl" class="flex-1 overflow-y-auto mb-3 pr-1 flex flex-col">
      <TransitionGroup
        v-if="assembledInfo.messages.length > 0"
        name="list"
        tag="div"
        class="relative"
      >
        ...
        >
          <div class="flex items-center justify-between mb-0.5">
            ...
            <span v-if="isChatMsg(i)" class="flex items-center gap-1">
              ...
              <button class="text-xs text-gray-500 hover:text-red-400" @click="deleteMessage(i)">
                ✕
              </button>
            </span>
          </div>
          <div
            v-if="isChatMsg(i) && isAssistantMsg(i) && !msg.content && sending"
